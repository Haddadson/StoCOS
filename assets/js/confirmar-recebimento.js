...
$(document).on('click', '#confirmarRecebimento', function (e) {
    ...
    $.get("http://localhost:4567/redecosmeticos/getById?id=" + id_rede, (dados_rede) => {
        ...
        if (dados_rede) {
            $('#listaRedes').append('<a class="list-group-item list-group-item-action" ' +
                'id="list-home-list' + lote.id + '" data-toggle="list" href="#list-home" role="tab" aria-controls="home" data-id-lote=' + lote.id + ' data-id-rede=' + id_rede + '>' + dados_rede[0].nome + ": Data de Agendamento - " + lote['data-agendamento'] + '</a>');
